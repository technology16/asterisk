...
static const char app[] = "AMDFILE";

#define STATE_IN_WORD       1
#define STATE_IN_SILENCE    2

 /* Some default values for the algorithm parameters. These defaults will be overwritten from amdfile.conf */
 ...
 /* Set to the lowest ms value provided in amdfile.conf or application parameters */
 static int dfltMaxWaitTimeForFrame  = 50;

/* Set the status and cause on the channel */
 static void setVariables(struct ast_channel *chan, char *amdStatus, char *amdCause)
 {
 ...
 static void isAnsweringMachine(struct ast_channel *chan, const char *data)
 {
	...
	char *back = ast_strip(args.argFileName);
	char *front;
 	do {
		ast_stopstream(chan);

		if ( !(front = strsep(&back, "&")) ) {
			ast_verb(3, "AMDFILE: No files left to playing on %s.\n", ast_channel_name(chan));
			break;
		}

		/* Turn on stream from file to channel */
		if (ast_streamfile(chan, front, ast_channel_language(chan))) {
			ast_log(LOG_WARNING, "AMDFILE: ast_streamfile failed on %s for %s\n", ast_channel_name(chan), (char *)front);
			break;
		}
				...
				if (!isAnalized) {
					if (f->frametype == AST_FRAME_VOICE || f->frametype == AST_FRAME_NULL || f->frametype == AST_FRAME_CNG) {
						...
						if (iTotalTime >= totalAnalysisTime) {
							...
							setVariables(chan, amdStatus, amdCause);
							isAnalized = 1;
						}
 ...
 static int amdfile_exec(struct ast_channel *chan, const char *data)
 {
 ...
 static int load_config(int reload)
 {
 	...
 	if (!(cfg = ast_config_load("amdfile.conf", config_flags))) {
 		ast_log(LOG_ERROR, "Configuration file amdfile.conf missing.\n");
 		return -1;
 	} else if (cfg == CONFIG_STATUS_FILEUNCHANGED) {
 		return 0;
 	} else if (cfg == CONFIG_STATUS_FILEINVALID) {
 		ast_log(LOG_ERROR, "Config file amdfile.conf is in an invalid format.  Aborting.\n");
 		return -1;
 	}
 ...
 static int load_module(void)
 {
 	...
 	if (ast_register_application_xml(app, amdfile_exec))
 ...
 AST_MODULE_INFO(ASTERISK_GPL_KEY, AST_MODFLAG_DEFAULT, "Answering Machine Detection Application with file playback on background",
 		.load = load_module,
